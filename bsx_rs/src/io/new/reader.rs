...
use log::warn;
use crate::io::new::bsx_batch::BsxBatch;
use crate::io::new::report_batch::DataBatch;
...
impl ReportReaderBuilder {
    ...
    pub fn with_report_type(mut self, report_type: ReportSchema) -> Self {
    ...
    fn check_fasta_path(&self) -> Result<(), Box<dyn Error>> {
        if self.fasta_path.is_some() && self.fai_path.is_some() {
            ...
            Err(Box::from("Both FASTA and FASTA index \
            paths should be specified"))
        }
    }
    fn check_type_and_fasta(&self) -> Result<bool, Box<dyn Error>> {
        if *self.report_type.get().need_align() || self.fasta_path.is_some() {
...
pub(crate) fn get_reader<F>(handle: F, report_type: ReportSchema) -> CsvReader<F>
where
...
impl ReportReader
{
    ...
    fn get_context_data(region: RegionCoordinates) -> ContextData {
        todo!()
    }
    
    fn process_batch(&mut self, batch: DataFrame) -> PolarsResult<BsxBatch>  {
        let report_schema = self.report_schema.get();
        let batch = report_schema.bsx_schema_mutate(batch.lazy());
        let do_align = report_schema.need_align().clone();
