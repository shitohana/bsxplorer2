...
use polars::prelude::*; // Added for testing DataFrame creation
...
mod tests {
    use super::*;
    use crate::data_structs::batch::BsxTypeTag;
    ...
    fn test_new() {
        ...
        let data = ContextData::new(positions.clone(), strands.clone(), contexts.clone());
    ...
    fn test_from_sequence_basic() {
        ...
        // Shift 3 (pos 4): TAG
        //   [_,_,G] [0]!=C && [1]!=C (T!=C && A!=C): Yes. Entry(1+3+2, R, CHH) -> (6, R, CHH).
        // Suffix "AG": No CG.
        ...
        assert_eq!(s, vec![Strand::Forward, Strand::Forward, Strand::Reverse, Strand::Reverse]);
        assert_eq!(c, vec![Context::CHG, Context::CG, Context::CG, Context::CHH]);
    }
    ...
    fn test_read_sequence_start_position() {
        ...
        // New positions (start=10): 10+(1-1)=10, 10+(2-1)=11, 10+(3-1)=12, 10+(6-1)=15
        // Expected entries: (10, F, CHG), (11, F, CG), (12, R, CG), (15, R, CHH)
        // Sorted: (10, F, CHG), (11, F, CG), (12, R, CG), (15, R, CHH)
    ...
    fn test_read_sequence_suffix_cg() {
        ...
        // Shift 1 (pos 2): TCG
        //   [C,G,_]: Yes. Entry(1+1, F, CG) -> (2, F, CG). Entry(1+1+1, R, CG) -> (3, R, CG).
        // Suffix "CG" at end (len=4, index 3): Yes. Pos 1 + 4 - 1 = 4. Entry(4, R, CG).
        // Unique entries: (1, F, CHH), (2, F, CG), (3, R, CG), (4, R, CG)
        // Sorted: (1, F, CHH), (2, F, CG), (3, R, CG), (4, R, CG)
        // Pos: 1, 2, 3, 4
        // Strands: F, F, R, R
    ...
    fn test_drain_until() {
        let mut data = ContextData::new(
            vec![10, 20, 30, 40, 50],
            vec![Strand::Forward, Strand::Reverse, Strand::Forward, Strand::Reverse, Strand::Forward],
            vec![Context::CG, Context::CHG, Context::CHH, Context::CG, Context::CHG],
        );
    ...
    fn test_take() {
        ...
        // Ensure the original struct is effectively empty after take (data is moved out)
        // Cloning before take is needed if we want to check the original struct afterwards
    ...
    fn test_to_df_empty() {
        ...
        let df_decoded = data.clone().to_df::<MockDecodedBsxBatch>();
