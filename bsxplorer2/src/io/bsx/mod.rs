...
//!
//! - [`BsxFileReader`]: A reader optimized for parallel access to data batches
//!   within a `.bsx` file, leveraging memory mapping and multithreading.
//! - [`BsxFileWriter`]: A writer for creating `.bsx` files from data batches,
//!   supporting different data sources for schema information (e.g., FASTA
//!   index) and optional compression.
...
//!   index is associated with the `.bsx` file and is used by the
//!   [`RegionReader`].
...
//!
//! The module utilizes `polars-arrow` for the core Arrow IPC
//! serialization/deserialization, `memmap2` for efficient file mapping, and
