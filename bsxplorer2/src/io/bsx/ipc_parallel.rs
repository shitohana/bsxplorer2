...
impl BsxFileReader {
    fn try_new<R: AsRawFd + 'static>(handle: R) -> anyhow::Result<Self> {
    ...
    pub fn get_batches(
        ...
        self.thread_pool.install(|| {
            batch_indices
                .chunks(self.thread_local_handles.len())
                .map(|tasks_chunk| {
    ...
    pub fn cache_batches(
        ...
        let mut new_batches = self.get_batches(batch_indices)
            .into_iter()
    ...
    fn get_cache_mut(&mut self) -> &mut VecDeque<BsxBatch> {
        &mut self.cache
    }
    ...
    pub fn iter(&mut self) -> BsxFileIterator {
        BsxFileIterator { reader: self, current_batch: 0 }
    }
}

pub struct BsxFileIterator<'a> {
    reader: &'a mut BsxFileReader,
    current_batch: usize
}

impl<'a> Iterator for BsxFileIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(batch) = self.reader.get_cache_mut().pop_front() {
            ...
            let to_read = (self.current_batch..(self.current_batch + self.reader.n_threads()))
                .collect_vec();
...
mod tests {
    ...
    #[rstest]
    fn test_iter(mut reader: BsxFileReader) {
        let mut batch_count = 0;
        for batch in reader.iter() {
            assert!(batch.is_ok());
            batch_count += 1;
        }
        assert_eq!(batch_count, reader.blocks_total())
    }
}
