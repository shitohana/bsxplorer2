...
trait SegmentationData {
    fn cost_function(&self, r: usize, t: usize) -> f64;
    fn len(&self) -> usize;
}
...
impl MethDataBinom {
    pub fn new(count_m: &[u32], count_total: &[u32]) -> Self {
        assert_eq!(count_m.len(), count_total.len(), "count_m and count_total must have the same length");
        let count_m_cumsum = count_m.iter()
                .fold(vec![0], |mut acc, new| {acc.push(acc.last().unwrap() + *new); acc});
        
        let count_total_cumsum = count_total.iter()
                .fold(vec![0], |mut acc, new| {acc.push(acc.last().unwrap() + *new); acc});
        
        Self { count_m_cumsum, count_total_cumsum }
    }
}
...
impl Ord for SegmentBoundary {
    fn cmp(&self, other: &Self) -> Ordering {
        self.chr_num.cmp(&other.chr_num).then(self.pos.cmp(&other.pos))
    }
}
