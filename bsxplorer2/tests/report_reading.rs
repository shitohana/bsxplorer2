...
fn compare_batches(original: &BsxBatch, read: &BsxBatch, report_type: &ReportTypeSchema) -> anyhow::Result<()> {
    if original.chr_val()? != read.chr_val()? {
        bail!("Chromosomes differ for original: {}\nread: {}", original.data(), read.data());
    }
    if original.height() != read.height() {
        bail!("Heights differ for original: {}\nread: {}", original.data(), read.data());
    }
    let (original_df, read_df) = if matches!(report_type, ReportTypeSchema::BedGraph) {
        (
            original.data().drop_many([COUNT_M_NAME, COUNT_TOTAL_NAME]),
            read.data().drop_many([COUNT_M_NAME, COUNT_TOTAL_NAME]),
        )
    } else {
        (original.data().clone(), read.data().clone())
    };
    if !original_df.equals_missing(&read_df) {
        let diff_mask = original_df.materialized_column_iter()
        ...
        bail!("Data differs for original: {}\nread: {}", orig_diff, read_diff);
    }
...
fn test_report_reading_with_alignment(
    ...
    for _chr in 0..N_CHR {
        ...
        let report_df = lazy_batch.as_report(&report_type)?;
    ...
    if do_alignment {
        report_reader_builder = report_reader_builder.with_fasta_path(sequence_file.path().to_path_buf());
    }
    
    // Build the report reader and create an iterator
    let report_reader = report_reader_builder
        .build_from_handle(Box::new(report_handle))?;
