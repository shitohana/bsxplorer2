...
///
/// This macro evaluates an expression that produces a collection, checks if all elements
/// are equal, and returns an appropriate Result.
///
/// # Arguments
///
    ...
    ($expression: expr, $err_string: expr) => {
        'eval: {
            ...
            if evaled.all_equal() {
                ...
                break 'eval Err(anyhow!($err_string).context(format!("Values: {:?}", evaled)));
            }
        }
    };
}
...
pub struct EncodedBsxBatchGroup<R: Display + Eq> {
    ...
    /// Optional labels associated with each batch (e.g., condition, tissue type, etc.)
    labels: Option<Vec<R>>,
}
    ...
    R: Display + Eq + Hash + Clone + Default,
{
    ...
    /// # Arguments
    ///
    /// * `batches` - A vector of `EncodedBsxBatch` instances representing samples.
    ...
    pub fn try_new(batches: Vec<EncodedBsxBatch>, labels: Option<Vec<R>>) -> anyhow::Result<Self> {
        ...
        .with_context(|| "All batches must reference the same chromosome/contig")?;
    ...
    /// It should only be used when the caller can guarantee that:
    ...
    /// # Arguments
    ///
    /// * `batches` - A vector of `EncodedBsxBatch` instances that are known to be consistent.
    ///
    /// # Returns
    ///
    ...
    ///
    /// This internal method applies the same mask to all batches, maintaining consistency.
    ///
    /// # Arguments
    ///
    /// * `mask` - A Boolean chunked array specifying which rows to keep (true) or filter out (false).
    ///
    /// # Returns
    ///
    /// A new `EncodedBsxBatchGroup` containing only the rows that match the mask.
    fn filter_mask(self, mask: &BooleanChunked) -> anyhow::Result<Self> {
        ...
        let filtered_batches =
            filtered_batches.with_context(|| "Failed to apply filter mask to batches")?;
    ...
    /// Filters all batches in the group to keep only rows with the specified genomic context.
    ///
    /// This method computes a Boolean mask once and applies it to all batches, which is more
    /// efficient than filtering each batch individually.
    ///
    /// # Arguments
    ///
    ...
    /// Filters all batches in the group to keep only rows with the specified DNA strand.
    ///
    /// This method computes a Boolean mask once and applies it to all batches, which is more
    /// efficient than filtering each batch individually.
    ///
    ...
    /// Replaces low coverage methylation sites with count_total = 0 and density = NaN.
    ///
    /// This method is useful for filtering out unreliable methylation measurements that don't
    /// have sufficient read coverage.
    ///
    /// # Arguments
    ///
    /// * `threshold` - The minimum coverage (read count) required. Sites with coverage below this value
    ///   will be marked as having zero total count and NaN density.
    ///
    /// # Returns
    ///
    ...
    /// Filters rows where the number of missing (NaN) methylation site densities is less than or equal to `n_missing`.
    ///
    /// This method is useful for removing sites that have too many missing values across samples.
    ///
    /// # Arguments
    ///
    /// * `n_missing` - The maximum number of missing values allowed for a site to be kept.
    ///   If a site has more than `n_missing` NaN values across all samples, it will be removed.
    ///
    /// # Returns
    ///
    ...
    pub fn filter_n_missing(self, n_missing: usize) -> anyhow::Result<Self> {
        ...
        if threshold > self.n_samples() {
            warn!(
                "Value for filter_n_missing ({}) exceeds sample count ({}). Adjusting to {}",
                threshold,
                self.n_samples(),
                self.n_samples()
            );
            ...
            let nan_vecs = self
                ...
                .map(|batch| {
                    Ok(batch
                        ...
                        .f32()
                        .with_context(|| "Failed to convert 'density' column to f32")?
                        .iter()
                        .map(|v| v.map(|x| x.is_nan()).unwrap_or(true))
                        .collect())
                })
                .collect::<anyhow::Result<Vec<Vec<bool>>>>()
                .with_context(|| "Failed to collect NaN vectors from batches")?;

            let (width, height) = (self.n_samples(), self.height());
            debug!(
                "Checking for missing values across {} samples with {} sites each",
                width, height
            );
    ...
    ///
    /// Assumes that all batches have the same height, which is enforced by `try_new`.
    ///
    /// # Returns
    ///
    ...
    ///
    /// If no labels were provided when creating this group, all batches will be assigned the default label.
    ///
    /// # Returns
    ///
    ...
    /// # Arguments
    ///
    /// * `na_rm` - If true, missing values (NaN) are ignored in the calculation.
    ///   If false, NaN values result in NaN averages.
    ///
    /// # Returns
    ///
        ...
        let res = (0..height)
            ...
            .map(|j| {
                ...
                if values.is_empty() && na_rm {
                    f64::NAN // Return NaN if all values were NaN and we're removing NaNs
                } else {
    ...
    pub fn get_sum_counts_m(&self) -> anyhow::Result<Vec<u32>> {
        ...
        let count_m_cols = self
            ...
            .collect::<anyhow::Result<Vec<_>, _>>()
            .with_context(|| "Failed to extract methylated counts from batches")?;
    ...
    ///
    /// Assumes that all batches have the same positions, which is enforced by batch group creation.
    ///
    /// # Returns
    ///
    ...
    pub fn get_positions(&self) -> anyhow::Result<Vec<u32>> {
        ...
        self.batches[0]
            .get_position_vals()
            .with_context(|| "Failed to retrieve position values from first batch")
    }

    /// Retrieves the chromosome (contig) name.
    ///
    /// Assumes that all batches have the same chromosome, which is enforced by batch group creation.
    ///
    /// # Returns
    ///
    ...
    pub fn get_chr(&self) -> anyhow::Result<String> {
        ...
        self.batches[0]
            .chr()
            .with_context(|| "Failed to retrieve chromosome name from first batch")
    }
}
