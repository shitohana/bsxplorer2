...
pub struct DmrConfig {
    ...
    pub min_cpgs: usize,
    pub segment_model: SureSegmentModelConfig,
}
...
impl Default for DmrConfig {
    fn default() -> Self {
        Self {
            ...
            diff_threshold: 0.1,
            min_cpgs: 5,
            segment_model: SureSegmentModelConfig::default(),
        }
    }
}
    ...
    R: Display + Eq + Hash + Clone + Default + std::fmt::Debug,
{
    ...
    fn get_intersecting_indices(
        ...
        let segments_left = region_left.segment_indices_sure(&self.config.segment_model);
        let segments_right = region_right.segment_indices_sure(&self.config.segment_model);
    ...
    R: Display + Eq + Hash + Clone + Default + Debug,
{
    fn bbinom_test(
        ...
        let alt = beta_binom::fit_alt_model(
            left_obs,
            right_obs,
            config.base_alt,
            config.epsilon,
        ...
        if let (Ok(null), Ok(alt)) = (null, alt) {
            let p_value = beta_binom::likelihood_ratio_test(null, alt, 1);
            Ok(DMRegion::new(
                chr,
