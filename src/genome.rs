...
impl AnnotationFileConf {
    ...
    pub fn col_names(&self) -> Vec<String> {
        [
            Some("chr"), 
            if self.strand_col.is_some() { Some("strand") } else { None },
            Some("start"), 
            Some("end"),
            if self.type_col.is_some() { Some("type") } else { None },
            if self.id_col.is_some() { Some("id") } else { None },
        ].iter().flatten().map(|s| s.to_string()).collect::<Vec<_>>()
...
impl Annotation {
    ...
    pub fn from_custom(
        ...
        let mut data = LazyCsvReader::new(file)
            ...
            .with_comment_prefix(Some(PlSmallStr::from(configuration.comment_prefix.clone().unwrap_or("#".to_string()))))
            .with_try_parse_dates(false)
            .with_has_header(configuration.has_header.unwrap_or(false))
            .finish()
            .expect("could not open file");
        
        let schema_cols = data.collect_schema().expect("schema generation failed");
        let selector = configuration.col_indices().iter()
            .zip(configuration.col_names())
            .map(|(idx, name)| -> Expr {
                let old_name = schema_cols.get_at_index(*idx).unwrap().0.to_owned();
                col(old_name).cast(self_schema.get_field(&name).unwrap().dtype).alias(name)
            }).collect::<Vec<_>>();
        let raw = data.select(&selector).filter(configuration.read_filters.unwrap_or(lit(true))); 
        Self {raw}
    }
